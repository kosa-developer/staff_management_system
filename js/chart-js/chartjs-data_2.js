...
$(document).ready(function () {
    ...
    for (var i = 0; i < uniqueCategoriesData.length; i++) {
        labels.push(uniqueCategoriesData[i].category_name);
        var year_data_got = uniqueCategoriesData[i].category_data;
        var levelCategories=[];
        for (var x = 0; x < year_data_got.length; x++) {
            if (!categoryLabels.includes(year_data_got[x].Category)) {
                categoryLabels.push(year_data_got[x].Category);
            }
            levelCategories.push(year_data_got[x].Survey);
        }
        dataSetsData.push({
